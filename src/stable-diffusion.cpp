...
#include "tokenizers/hf_tokenizer_json.h"
...
// hunyuan.hpp and lingbot_video.hpp must come after ltxv.hpp: they pull in
// mmdit.hpp's global modulate(), which is ambiguous with LTXV::modulate()
// under ADL if visible while ltxv.hpp's unqualified calls are compiled.
...
public:
    ...
    bool init(const sd_ctx_params_t* sd_ctx_params) {
        ...
        if (strlen(SAFE_STR(sd_ctx_params->tokenizer_pack_path)) > 0) {
            VocabProvider::instance().set_pack_dir(sd_ctx_params->tokenizer_pack_path);
        } else if (getenv("SD_TOKENIZER_PACK") != nullptr && strlen(getenv("SD_TOKENIZER_PACK")) > 0) {
            VocabProvider::instance().set_pack_dir(getenv("SD_TOKENIZER_PACK"));
        }
        ...
        // Some models embed their LLM tokenizer.json as a tensor; use it as
        // the tokenizer source unless the user injected a pack explicitly
        // (the pack dir has priority inside the VocabProvider). Must happen
        // before the conditioner is constructed, where tokenizers are built.
        if (tensor_storage_map.find("text_encoders.llm.tokenizer_json") != tensor_storage_map.end()) {
            std::string vocab_family;
            if (sd_version_is_lens(version)) {
                vocab_family = "gpt_oss";
            } else if (sd_version_is_ideogram4(version)) {
                vocab_family = "qwen2";
            }
            if (!vocab_family.empty()) {
                std::string tokenizer_json;
                if (model_loader.read_tensor_bytes("text_encoders.llm.tokenizer_json", tokenizer_json)) {
                    std::string merges;
                    std::string vocab_json;
                    if (hf_tokenizer_json_extract_merges(tokenizer_json, merges)) {
                        VocabProvider::instance().set_override(vocab_family + ".merges", std::move(merges));
                    }
                    if (hf_tokenizer_json_extract_vocab(tokenizer_json, vocab_json)) {
                        VocabProvider::instance().set_override(vocab_family + ".vocab", std::move(vocab_json));
                    }
                } else {
                    LOG_WARN("failed to read embedded tokenizer_json, falling back to built-in tokenizer data");
                }
            }
        }
            ...
            auto create_vae = [&]() -> std::shared_ptr<VAE> {
                if (sd_version_is_ltxav(version)) {
                    return std::make_shared<LTXVideoVAE>(backend_for(SDBackendModule::VAE),
                                                         ...
                                                         version,
                                                         model_manager);
                } else if (sd_version_uses_hunyuan_video_vae(version)) {
