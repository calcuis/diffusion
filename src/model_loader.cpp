...
bool ModelLoader::load_tensors(on_new_tensor_cb_t on_new_tensor_cb,
    ...
    for (size_t file_index = 0; file_index < file_data.size(); ++file_index) {
        ...
        int n_threads = std::min(num_threads_to_use, (int)tensors_to_process.size());
        if (n_threads < 1) {
            n_threads = 1;
        }
...
bool ModelLoader::read_tensor_bytes(const std::string& name, std::string& out) {
    ...
    if (!file.is_open()) {
        LOG_ERROR("read_tensor_bytes: failed to open '%s'", file_path.c_str());
        return false;
    }
    int64_t nbytes = tensor_storage.nbytes_to_read();
    out.resize(static_cast<size_t>(nbytes));
    file.seekg(tensor_storage.offset, std::ios::beg);
    file.read(&out[0], nbytes);
    if (!file) {
        LOG_ERROR("read_tensor_bytes: failed to read %" PRId64 " bytes at offset %" PRIu64 " from '%s'",
                  nbytes, tensor_storage.offset, file_path.c_str());
        out.clear();
        return false;
    }
    return true;
