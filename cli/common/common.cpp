...
ArgOptions SDContextParams::get_options() {
    ...
    options.string_options = {
        ...
        {"",
         "--tokenizer-pack",
         "path to a dir with replacement tokenizer data, <family>/{merges.txt|vocab.json|tokenizer.json}; overrides the built-in data (env: SD_TOKENIZER_PACK)",
         0,
         &tokenizer_pack_path},
        {"",
         "--diffusion-model",
...
sd_ctx_params_t SDContextParams::to_sd_ctx_params_t(bool taesd_preview) {
    ...
    sd_ctx_params.tokenizer_pack_path             = tokenizer_pack_path.c_str();
...
std::string version_string() {
    return std::string("diffusion version ") + sd_version() + ", commit " + sd_commit();
}
...
std::string get_image_params(const SDContextParams& ctx_params,
    ...
    parameter_string += "Version: diffusion.cpp";
